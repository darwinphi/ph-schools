...
const sampleRecord = {
  ...
  "Sector": "Public",
  "Urban/Ru": "Urban",
  "Sacl hColaosls Sifuicbactliaosnsification": "DepED Managed",
...
test("generate-data succeeds using pinned remote data source", async () => {
  ...
  assert.match(generated, /export const sourceFile = "schools_masterlist_2020_2021\.json"/);
  assert.match(generated, /export const rawRecords = parseRawRecords/);
});

test("generate-data fails with checksum mismatch", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "ph-schools-generate-checksum-"));
...
test("generate-data fails when remote source cannot be downloaded", async () => {
  ...
  assert.notEqual(result.status, 0);
  assert.match(`${result.stderr}\n${result.stdout}`, /Failed to download dataset/);
